...
import br.ufpa.linc.xflow.presentation.view.SingleProjectView;
	...
	public int getColor(VisualItem item) {
		...
		if ( item instanceof NodeItem ) {
			...
			if ( m_vis.isInGroup(item, Visualization.SEARCH_ITEMS) ) {
				for(int i=0; i < ((SingleProjectView) Visualizer.getProjectsViewer()).developersPanelControl.getDevelopersList().length; i++){
					if(((SingleProjectView) Visualizer.getProjectsViewer()).developersPanelControl.getCheckBoxList().getSelectionModel().isSelectedIndex(i)){
